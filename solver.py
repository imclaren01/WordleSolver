...
def reorderWords():
    ...
    ng = sorted(list(filter(lambda x: correctYellows(x[0]),filter(lambda x: not anyKnownNot(x[0]), filter(lambda x: hasGreens(x[0]), filter(lambda x: allKnown(x[0]), WORD_LIST))))), key= lambda x: scoreGuess(x[0]), reverse=True)
...
def main():
    ...
    yellowsTemp = [item.split(',') for item in input("First, guess 'ORATE'. Which letters (if any) are now yellow? Please enter in format 'c1,n1 c2,n2' where c1 is the first yellow character and n1 is its location in the word from 1 to 5: ").split()]
    YELLOWS.update({x[0]: int(x[1])-1 for x in yellowsTemp})
    KNOWN = set([x[0] for x in yellowsTemp])
    if input("Was this guess correct? (answer 'y' or 'n'): ") == 'y':
    ...
    while len(GUESSES) > 1:
        ...
        if correct:
            ...
            sys.exit()
        print(GUESSES)
    print(f"Based on what you've told me, '{GUESSES[0][0].upper()}'' should be the correct word. Sorry if it's not!" )
    sys.exit()
